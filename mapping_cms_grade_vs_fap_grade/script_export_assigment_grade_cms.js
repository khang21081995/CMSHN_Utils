...
let myExcelXML = (function () {
    ...
    class myExcelXML {
        constructor(o,file='export') {
            let respArray = typeof o === 'string' ? JSON.parse(o) : o;
            let finalDataArray = [];
            fileName = file;
...
while ($(`#mod_assign_grading_r${row}_c3`).text()) {
    let a = $(`#mod_assign_grading_r${row}_c5`).text();
    data.push({
        studentCode: ($(`#mod_assign_grading_r${row}_c3`).text().split("@")[0] + "").toLowerCase(),
        studentGrade: +a.substring(5, a.length - 9)
    })
    row++;
}

var myTestXML = new myExcelXML(data,"student_grade_export");
myTestXML.downLoad();